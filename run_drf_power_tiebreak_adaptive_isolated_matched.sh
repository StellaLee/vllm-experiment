#!/bin/bash
# Hypothesis test: does replacing the fixed ramp_ceiling_w_per_s=450 constant with a
# self-calibrating (rolling p99, floored at 450) ceiling change anything on the SAME
# throughput-matched, cache-hit-rate~0 whale-injection condition drf_power_tiebreak was
# validated on? This condition's own natural regime is close to what 450 was calibrated
# for, so the adaptive ceiling should track close to the floor here -- the real test of
# generalization is the paired cachehit (light-load, cache-hit-rate>0) run.
# Baseline to beat: drf_power_tiebreak max_ramp 3015.3+-257.1, coincidence 4.4+-1.2.
set -x
...
for TRIAL in 1 2 3; do
  echo "=== DRFADAPTIVEISOLATEDMATCHED BATCH: policy=$OUTNAME trial=$TRIAL ==="
  ...
  pkill -f "power_logger.py" 2>/dev/null
  sleep 3
  echo "=== DRFADAPTIVEISOLATEDMATCHED BATCH: policy=$OUTNAME trial=$TRIAL COMPLETE ==="
done
echo "=== FULL DRFADAPTIVEISOLATEDMATCHED BATCH COMPLETE ==="
