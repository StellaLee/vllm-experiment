#!/bin/bash
# Extends weighted_sum_coincidence_ceiling and lmetric_power_coincidence_ceiling (validated
# so far only on Heavy/Closed-Loop's long condition) to the remaining 6 conditions: BurstGPT,
# Heavy/Matched, Light/Cachehit, Heavy/Closed-Loop (short), Ramp & Route, WildChat -- matching
# the same expansion already done for the DRF-family drf_power_tiebreak_full_coincidence_
# ceiling, so all three coincidence-ceiling variants are comparable across the same condition
# set. Same structure/helpers as run_pergpu_coincidence_ceiling_6conditions.sh; harness args
# copied verbatim from run_pergpu_4arms_5conditions.sh and run_burstgpt_perGPU_safetriad.sh.
# 2 arms x 6 conditions x 3 trials = 36 runs, sequential.
set -x
...
ARMS=("weighted_sum_coincidence_ceiling weighted_sum_coincidence_ceiling" \
...
for PAIR in "${ARMS[@]}"; do
  ...
  for TRIAL in 1 2 3; do
    run_condition_trial openloopwhalelongoutmatched data/sharegpt_v3.json "$OUTNAME" "$POLICY" "$TRIAL" \
  ...
  for TRIAL in 1 2 3; do
    run_condition_trial cachehit data/sharegpt_v3.json "$OUTNAME" "$POLICY" "$TRIAL" \
      --pad-chars 0 --whale-frac 0.0 --min-turns 2 --max-turns 4 \
      --concurrency 24 --num-convs 150 --max-tokens 128
  done

  for TRIAL in 1 2 3; do
    run_condition_trial closedloopheavy data/sharegpt_v3.json "$OUTNAME" "$POLICY" "$TRIAL" \
      --min-turns 1 --max-turns 1 --concurrency 32 --num-convs 150 --max-tokens 1024 \
      --whale-frac 0.15 --whale-min-chars 44000 --whale-max-chars 50000
  done

  for TRIAL in 1 2 3; do
    run_condition_trial rampandroute data/sharegpt_v3.json "$OUTNAME" "$POLICY" "$TRIAL" \
      --min-turns 1 --max-turns 1 --concurrency 24 --num-convs 150 --max-tokens 128 \
      --whale-frac 0.15 --whale-min-chars 44000 --whale-max-chars 50000
  done

  for TRIAL in 1 2 3; do
    run_condition_trial wildchatnatural data/wildchat_v1.json "$OUTNAME" "$POLICY" "$TRIAL" \
      --min-turns 1 --max-turns 4 --rate 10.7 --num-convs 750 --max-tokens 1024
  done
done

echo "=== WS_LP COINCIDENCE_CEILING 6-CONDITION BATCH COMPLETE ==="
