#!/bin/bash
# BurstGPT at 1.5x compressed arrival timing (--trace-rate-scale 1.5), same 4 arms as the
# other rate-sweep batteries, testing whether the coincidence_ceiling vs round_robin
# TTFT/TBT pattern (findings.md Parts 18-19) generalizes to a real-trace condition under
# higher load. IMPORTANT: this is NOT unmodified real-trace replay -- inter-arrival gaps from
# burstgpt_heavy_trace.csv are compressed 1.5x, so timing is synthetic even though prompt/
# response token counts and their order are the real trace's own. Must be reported as
# "BurstGPT, 1.5x rate-scaled", never as the plain real trace.
set -x
...
run_trial () {
  ...
  echo "=== running harness (${PREFIX} policy=$OUTNAME trial=$TRIAL scale=$SCALE) ==="
...
echo "=== BURSTGPT_RATESCALE15_BATCH COMPLETE ==="
