#!/bin/bash
# Elevation-count ablation: same D and tie-break as drf_power_tiebreak_full_coincidence_ceiling
# (both real, unrandomized), but coincidence_ceiling_factor's n_elevated is read from
# LAGGED_ELEVATION_DEPTH=75 decisions ago instead of the current candidate set -- decorrelates
# WHEN the ceiling squeeze fires from whether a real simultaneous-pressure event is actually
# happening, while preserving the real signal's exact marginal distribution (same underlying
# process, time-shifted, not synthetic noise). Unlike the random-power-tiebreak ablation (which
# only randomizes the SECONDARY tie-break coordinate), this targets the PRIMARY-criterion
# mechanism directly -- the part with the real, already-documented wins (Part 2: best p99_ramp
# of 6 arms on Heavy/CL long; 0/453 avoidable violations).
#
# Same two conditions as run_pergpu_random_power_tiebreak.sh, n=6 each, for direct comparison
# against both the existing coincidence_ceiling data AND the random-tiebreak ablation's data on
# the same conditions.
set -x
...
OUTNAME=lagged_elevation
POLICY=drf_power_tiebreak_full_coincidence_ceiling_lagged_elevation
...
for TRIAL in 1 2 3 4 5 6; do
  run_trial closedloopheavylongpergpu "$TRIAL" \
...
for TRIAL in 1 2 3 4 5 6; do
  run_trial openloopwhalelongoutmatchedpergpu "$TRIAL" \
    --min-turns 1 --max-turns 1 --rate 10.7 --num-convs 750 --max-tokens 1024 \
    --whale-frac 0.15 --whale-min-chars 44000 --whale-max-chars 50000
done

echo "=== LAGGED_ELEVATION_BATCH COMPLETE ==="
