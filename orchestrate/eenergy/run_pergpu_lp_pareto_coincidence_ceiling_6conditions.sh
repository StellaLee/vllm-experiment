#!/bin/bash
# Extends lmetric_power_pareto_coincidence_ceiling (validated so far only on Heavy/Closed-
...
# expansion already done for the other three coincidence-ceiling variants, so all four are
# comparable across the same condition set. Same structure/helpers as
# run_pergpu_coincidence_ceiling_6conditions.sh; harness args copied verbatim from
...
OUTNAME=lmetric_power_pareto_coincidence_ceiling
POLICY=lmetric_power_pareto_coincidence_ceiling
...
echo "=== LP_PARETO_COINCIDENCE_CEILING 6-CONDITION BATCH COMPLETE ==="
