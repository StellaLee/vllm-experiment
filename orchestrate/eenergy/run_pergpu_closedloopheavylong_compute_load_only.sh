#!/bin/bash
# Isolates whether compute-avoidance, load-avoidance, or their combination is doing the
# implicit power/ramp-smoothing work observed in drf_no_power (which combines both via
# max()). Runs compute_only and load_only on Heavy/Closed-Loop's long condition, same params
# as run_pergpu_closedloopheavy_long5min.sh, for direct comparison against drf_no_power and
# every other arm already tested there.
...
echo "=== CLOSEDLOOPHEAVYLONG COMPUTE_LOAD_ONLY BATCH COMPLETE ==="
