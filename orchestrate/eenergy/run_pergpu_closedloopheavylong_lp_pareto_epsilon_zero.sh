#!/bin/bash
# Diagnostic-only run of lmetric_power_pareto_epsilon_zero_coincidence_ceiling (epsilon=0.0).
# NOT Pareto-safe -- expected to reproduce lmetric_power_coincidence_ceiling's routing
# decisions (the algebraic epsilon->0 limit of this family, not lmetric_power_pareto's
# epsilon->1 case), including Claim 2's cache-hit score-collapse failure mode. Checks the
# prediction from the epsilon-shrinking analysis directly. Heavy/Closed-Loop long, same
...
OUTNAME=lmetric_power_pareto_epsilon_zero_coincidence_ceiling
POLICY=lmetric_power_pareto_epsilon_zero_coincidence_ceiling
...
echo "=== LP_PARETO_EPSILON_ZERO BATCH COMPLETE ==="
