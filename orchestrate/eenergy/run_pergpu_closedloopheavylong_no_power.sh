#!/bin/bash
# Tests whether power-awareness in the routing decision itself actually helps: 3 power-blind
# arms (drf_no_power, weighted_sum_no_power, lmetric -- the latter already power-blind by
# construction, new_tokens x in_flight_after, reusing the existing "lmetric" policy) on
# Heavy/Closed-Loop's LONG (~225-232s) condition, same params as
# run_pergpu_closedloopheavy_long5min.sh, for direct comparison against the existing
# power-aware siblings already tested there (drf_fixed, weighted_sum, lmetric_power).
# 3 arms x 3 trials = 9 runs, sequential.
set -x
...
ARMS=("drf_no_power drf_no_power" "weighted_sum_no_power weighted_sum_no_power" "lmetric lmetric")
...
echo "=== CLOSEDLOOPHEAVYLONG NO_POWER BATCH COMPLETE ==="
