#!/bin/bash
# Extends the Heavy/Matched rate sweep (run_pergpu_matched_rate_sweep.sh) with the other two
# headline-adjacent arms: lmetric_power_coincidence_ceiling (strongest unsafe alternative) and
# weighted_sum_coincidence_ceiling (external validity check) -- same 2 rates (7.0, 15.0), same
# harness settings, 3 trials each. Chained to run after the first sweep (coincidence_ceiling,
# round_robin) so both share GPUs without colliding.
set -x
...
echo "=== MATCHED_RATE_SWEEP_EXT BATCH COMPLETE ==="
