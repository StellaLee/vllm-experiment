#!/bin/bash
# Extends lmetric_power_pareto_epsilon_small_coincidence_ceiling (epsilon=0.001, validated so
# far only on Heavy/Closed-Loop's long condition) to the remaining 6 conditions: BurstGPT,
# Heavy/Matched, Light/Cachehit, Heavy/Closed-Loop (short), Ramp & Route, WildChat -- matching
# the same expansion already done for the epsilon=0.01 variants. Same structure/helpers as
# run_pergpu_lp_pareto_epsilon_variants_6conditions.sh; harness args copied verbatim.
# 1 arm x 6 conditions x 3 trials = 18 runs, sequential.
set -x
...
OUTNAME=lmetric_power_pareto_epsilon_small_coincidence_ceiling
POLICY=lmetric_power_pareto_epsilon_small_coincidence_ceiling
...
echo "=== LP_PARETO_EPSILON_SMALL 6-CONDITION BATCH COMPLETE ==="
