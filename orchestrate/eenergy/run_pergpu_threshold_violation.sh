#!/bin/bash
# Collects the new avoidable-threshold-violation diagnostic (router_core.py's
# last_D_chosen/last_min_D_available/last_avoidable_threshold_violation, now logged in the
# assignment CSV) on the flagship condition (Heavy/Closed-Loop short), for the three arms
# Theorem 4/5 actually distinguishes: coincidence_ceiling (proven threshold-safe),
# weighted_sum_coincidence_ceiling (Pareto-safe but NOT threshold-safe per Theorem 5),
# lmetric_power_coincidence_ceiling (neither). Reuses the existing flagship condition rather
# than designing a new one -- this is a logging addition, not a new experimental regime.
set -x
...
echo "=== THRESHOLD_VIOLATION_BATCH COMPLETE ==="
