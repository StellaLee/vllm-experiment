#!/bin/bash
# First hardware validation of drf_power_tiebreak_full_coincidence_ceiling (this session's
# fleet-aggregate-targeting design, see scoring.py's coincidence_ceiling_factor docstring).
# Runs it on Heavy/Closed-Loop's LONG (~225-232s) variant -- same params as
# run_pergpu_closedloopheavy_long5min.sh -- since that's the condition where sustained
# duration gives coincidence events time to actually occur, and where the existing safe
# rules' story got messiest (weighted_sum losing dominance at 6 trials). Directly comparable
# to the existing closedloopheavylongpergpu_ data for all 5 other arms.
...
echo "=== CLOSEDLOOPHEAVYLONG COINCIDENCE_CEILING BATCH COMPLETE ==="
