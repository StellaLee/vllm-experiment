#!/bin/bash
# Duration-extension check for Heavy/Closed-Loop only: does the headline dominance result
# hold when the active workload window is ~5 minutes instead of the original ~51-53s?
# Same 5 arms (drf_fixed, drf_power_tiebreak_full, weighted_sum, lmetric_power, round_robin),
# same per-GPU-calibrated RAMP_CEILING_PER_GPU, same N_REPLICAS=6/GPU_OFFSET=2, same model,
# same concurrency (32) and whale-frac/whale-char-range as the original condition -- only
# --num-convs changes (150 -> 900, scaled from the measured ~2.94 convs/s throughput at
# concurrency=32 to target a ~300s active window). Prefix distinguishes this data
# (closedloopheavylongpergpu_*) from the original short-duration run
# (closedloopheavypergpu_*) so nothing is overwritten; the two are compared directly by
# scripts/eenergy/compare_closedloopheavy_duration.py.
# 5 arms x 3 trials = 15 runs, sequential (N_REPLICAS=6/GPU_OFFSET=2 saturates all 6 target
# GPUs, no cross-run parallelism possible on this 8-GPU box). Expected: ~300s active + ~43s
# setup/teardown per trial =~ 343s/trial x 15 =~ 86 minutes total.
set -x
...
echo "=== CLOSEDLOOPHEAVY 5-MINUTE DURATION-EXTENSION BATCH COMPLETE ==="
