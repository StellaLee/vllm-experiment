#!/bin/bash
# First hardware validation of weighted_sum_coincidence_ceiling and
# lmetric_power_coincidence_ceiling -- tests whether the coincidence-aware ceiling mechanism
# (validated so far only on drf_power_tiebreak_full_coincidence_ceiling, where it got the
# best p99_ramp of any arm tested) helps generally across rule families, or is specific to
# the DRF-family sorted rule's max()-first structure. Runs on Heavy/Closed-Loop's LONG
# (~225-232s) condition, same params as run_pergpu_closedloopheavy_long5min.sh, for direct
# comparison against all arms already tested there.
...
echo "=== CLOSEDLOOPHEAVYLONG WS_LP_COINCIDENCE_CEILING BATCH COMPLETE ==="
