#!/bin/bash
# Extends lmetric_power_coincidence_ceiling from 3 trials to 6 on the SAME arm, to establish
# a same-rule/same-content noise floor for the epsilon_zero comparison: t1 vs t2 already
# showed 712/901 (79%) per-request routing decisions differ despite identical rule + paired
# workload content, purely from closed-loop timing feedback. Trials 4-6 give a proper 6-trial
# variance estimate for this single arm to compare against, rather than relying on just a
# 2-trial (t1-vs-t2) spot check. Same condition/params as every other coincidence-ceiling
# variant's validation.
...
ARMS=("lmetric_power_coincidence_ceiling lmetric_power_coincidence_ceiling")
...
echo "=== CLOSEDLOOPHEAVYLONG LMETRIC_POWER_CC_SELF_REPLICATE TRIALS 4-6 BATCH COMPLETE ==="
