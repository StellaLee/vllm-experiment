#!/bin/bash
# Test drf_power_tiebreak on the concurrency=24 cache-hit ShareGPT workload (plain multi-turn,
# no pad, no whale) -- same conditions as findings.md's "Workload: plain ShareGPT, no pad, no
# whale, real multi-turn" section: --pad-chars 0 --whale-frac 0.0 --min-turns 2 --max-turns 4
# --concurrency 24 --num-convs 150 --max-tokens 128, pad-seed 12345 (default), N=6/GPUs 2-7/
# ramp-ceiling 450. Only this one new arm needs running -- the other 7 (round_robin, lmetric,
# drf_fixed, p2c_whale, whale_argmin, constrained_lmetric, pressure_switch) are already
# collected (cachehit_ prefix).
set -x
...
for TRIAL in 1 2 3; do
  echo "=== DRFPOWERTIEBREAKCACHEHIT BATCH: policy=$OUTNAME trial=$TRIAL ==="
  ...
  pkill -f "power_logger.py" 2>/dev/null
  sleep 3
  echo "=== DRFPOWERTIEBREAKCACHEHIT BATCH: policy=$OUTNAME trial=$TRIAL COMPLETE ==="
done
echo "=== FULL DRFPOWERTIEBREAKCACHEHIT BATCH COMPLETE ==="
