#!/bin/bash
# WildChat at a higher rate (15.0 vs the existing 10.7), same 4 arms as the Heavy/Matched rate
# sweep, testing whether the coincidence_ceiling vs round_robin TTFT/TBT pattern (findings.md
# Parts 18-19) generalizes to a second open-loop condition and to higher load. WildChat is
# open-loop with a real --rate knob (unlike BurstGPT's fixed trace-csv replay), so this is a
# direct, unmodified extension of the existing condition -- no methodology caveat needed.
set -x
...
echo "=== WILDCHAT_RATE15_BATCH COMPLETE ==="
