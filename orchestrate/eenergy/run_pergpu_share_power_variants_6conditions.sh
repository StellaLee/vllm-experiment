#!/bin/bash
# Extends both new Share_power definitions (drf_power_tiebreak_full_coincidence_ceiling_
# smoothed_ramp and drf_peak_power_tiebreak_full, validated so far only on Heavy/Closed-
# Loop's long condition) to the remaining 6 conditions: BurstGPT, Heavy/Matched,
# Light/Cachehit, Heavy/Closed-Loop (short), Ramp & Route, WildChat -- matching the same
# expansion pattern already used for every other coincidence-ceiling-family variant. Same
# structure/helpers as run_pergpu_lp_pareto_epsilon_variants_6conditions.sh; harness args
# copied verbatim.
...
ARMS=("drf_power_tiebreak_full_coincidence_ceiling_smoothed_ramp drf_power_tiebreak_full_coincidence_ceiling_smoothed_ramp" \
      "drf_peak_power_tiebreak_full drf_peak_power_tiebreak_full")
...
echo "=== SHARE_POWER_VARIANTS 6-CONDITION BATCH COMPLETE ==="
