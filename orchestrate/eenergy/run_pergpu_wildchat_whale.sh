#!/bin/bash
# Tests whether the Heavy family's long-context (whale-injected) content, not just its
# synthetic ShareGPT source, is what's driving coincidence_ceiling's clean results there vs.
# WildChat/BurstGPT's real losses (findings.md Parts 22-25). Same whale settings as every
# Heavy condition (--whale-frac 0.15 --whale-min-chars 44000 --whale-max-chars 50000),
# applied to real WildChat conversations instead of ShareGPT -- isolates content length from
# trace source. Same rate (10.7) as the original WildChat baseline for a clean before/after.
# All 5 headline-adjacent arms, 3 trials each.
set -x
...
echo "=== WILDCHAT_WHALE_BATCH COMPLETE ==="
