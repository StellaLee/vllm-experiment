#!/bin/bash
# Extends both epsilon-shift Pareto-safe lmetric_power variants
# (lmetric_power_pareto_epsilon_coincidence_ceiling and
# lmetric_power_pareto_epsilon_all_coincidence_ceiling, validated so far only on Heavy/
# Closed-Loop's long condition) to the remaining 6 conditions: BurstGPT, Heavy/Matched,
# Light/Cachehit, Heavy/Closed-Loop (short), Ramp & Route, WildChat -- matching the same
# expansion already done for every other coincidence-ceiling variant. Same structure/helpers
# as run_pergpu_coincidence_ceiling_6conditions.sh; harness args copied verbatim from
# run_pergpu_4arms_5conditions.sh and run_burstgpt_perGPU_safetriad.sh.
...
echo "=== LP_PARETO_EPSILON_VARIANTS 6-CONDITION BATCH COMPLETE ==="
