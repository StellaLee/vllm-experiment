#!/bin/bash
# Re-runs drf_no_power on Heavy/Closed-Loop's long condition with the new share_compute/
# share_load assignment-log columns (existing drf_no_power logs predate this instrumentation).
# Tests whether the dominant resource (compute vs load) for the CHOSEN candidate switches
# decision-to-decision more often than a single-resource rule would, which would explain
# drf_no_power's worse p99_ramp despite a LOWER absolute max ramp and at-least-as-even whale
# placement compared to compute_only/load_only (both checked and ruled out this session).
...
echo "=== DRF_NO_POWER DIAGNOSTIC BATCH COMPLETE ==="
