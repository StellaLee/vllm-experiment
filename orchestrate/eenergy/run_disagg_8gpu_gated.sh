#!/bin/bash
# Gated 4:4 P/D-disaggregated battery -- identical topology/workload to
# run_disagg_8gpu_baseline.sh, with both pool caps enabled this time. Cap values are computed
# from that baseline's measured sustained mean power per pool (Task 5 Step 1: cap = round(
# 0.925 * mean), mirroring the colocated gate's own 1800W/~1945W ratio for a comparable-
# aggressiveness test). Baseline measured: prefill mean=587.8W, decode mean=895.4W ->
# cap_prefill_w=544, cap_decode_w=828.
set -x
...
IDLE_SETTLE_S=30
COOLDOWN_S=10
# Filled in from Task 4's baseline mean_power_w output (Task 5 Step 1's formula:
# round(0.925 * mean)) -- computed once the baseline battery had actually run, not guessed.
CAP_PREFILL_W=544
CAP_DECODE_W=828
...
echo "=== launching gate shim on :${GATE_PORT} (prefill cap=${CAP_PREFILL_W}W decode cap=${CAP_DECODE_W}W) ==="
...
echo "=== DISAGG_8GPU_GATED_BATCH COMPLETE ==="
