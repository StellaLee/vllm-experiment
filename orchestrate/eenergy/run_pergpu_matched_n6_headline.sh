#!/bin/bash
# Replicates Heavy/Matched (openloopwhalelongoutmatchedpergpu) from n=3 to n=6 for all 4
# headline arms: coincidence_ceiling, lmetric, round_robin, lmetric_power_coincidence_ceiling.
# Trials 4-6 only (1-3 already exist). Theory-motivated pick, not outcome-selected -- see
# findings.md Part 18: this is one of the project's two sustained-pressure, multi-replica-
# elevation conditions, the regime the coincidence-ceiling mechanism is designed for.
# Same pad-seed (default 12345) as every existing trial -- do not vary content here, that's a
# separate cross-seed robustness check if/when we get to it.
set -x
...
echo "=== MATCHED_N6_HEADLINE BATCH COMPLETE ==="
