#!/bin/bash
# orchestrate_cs2_whale_threshold_replicate.sh -- Phase 1 replication of the §5.8 tradeoff
# (2026-07-23 finding). Same mechanism/workload as orchestrate_cs2_whale_threshold_sweep.sh,
# but: (a) conc=40 dropped (already confirmed overloaded, not part of what we're validating),
# (b) TRIAL env var tags output files so trial 1 (already on disk) is never overwritten,
# (c) same PAD_SEED as trial 1 -- isolates serving/timing noise (the thing "single trial"
# actually worries about) from workload-draw variance (which the whale-size/fraction grid,
# Phase 2, addresses separately).
set -uo pipefail
...
TRIAL=${TRIAL:?must set TRIAL=2 or TRIAL=3}
rm -f "logs/cs2wtrep_t${TRIAL}_ALLDONE" "logs/cs2wtrep_t${TRIAL}_FAILED"
log "cs2 whale/threshold replication TRIAL=$TRIAL: budget(fixed)=$BUDGET thresholds=[$THRESHOLDS] whale_fracs=[$WHALE_FRACS] conc=$CONC max_seqs=$MAX_SEQS nconv=$NCONV whale=[$WHALE_MIN,$WHALE_MAX]chars seed=$PAD_SEED"
...
run_threshold(){ # threshold
  ...
  env CUDA_VISIBLE_DEVICES=0,1 PREFIX_REORDER=0 DYNAMIC_CHUNK=0 $PYTHON -m vllm.entrypoints.openai.api_server \
      ...
      > logs/${DATE}-cs2wtrep${TRIAL}-${arm}-server.log 2>&1 &
  local sv=$!
  for i in $(seq 1 120); do sleep 5
    grep -q "Application startup complete" logs/${DATE}-cs2wtrep${TRIAL}-${arm}-server.log && break
  ...
  for wf in $WHALE_FRACS; do
    ...
    local out="logs/${DATE}-cs2wtrep${TRIAL}-${arm}-w${wtag}-c${CONC}-t${TRIAL}.jsonl"
    ...
    log "  [$arm wf=${wf} trial=${TRIAL}] done recs=$(grep -c . "$out" 2>/dev/null || echo 0) " \
        "preempt=$(grep -c -i preempt logs/${DATE}-cs2wtrep${TRIAL}-${arm}-server.log 2>/dev/null || echo 0)"
  done
...
if [ "$RC" = 0 ]; then touch "logs/cs2wtrep_t${TRIAL}_ALLDONE"; else touch "logs/cs2wtrep_t${TRIAL}_FAILED"; fi
log "trial $TRIAL done (rc=$RC)"
