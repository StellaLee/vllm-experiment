...
run_static(){ # $1=arm-label $2...=extra server CLI flags
  ...
  local SV=$!
  wait_up "$FB" || { log "  [$ARM] SERVER TIMEOUT"; kill $SV 2>/dev/null; sleep 8; kill -9 $SV 2>/dev/null; kill_ours; touch logs/ratesweep_fsx_FAILED; return 1; }
  replay "$ARM"
  kill $SV 2>/dev/null; sleep 8; kill -9 $SV 2>/dev/null; kill_ours
}

run_fairshare_both(){ # $1=arm-label
  local ARM=$1
  local FB="logs/${DATE}-lgate-b${ARM}"
  rm -f "${FB}-t1.jsonl" "${FB}-chunktrace.csv"
  log "  [$ARM] starting server (FAIRSHARE_BUDGET)"
  env CUDA_VISIBLE_DEVICES=2,3 PREFIX_REORDER=0 DYNAMIC_CHUNK=0 \
    FAIRSHARE_BOTH=1 FAIRSHARE_BOTH_TRACE="${FB}-chunktrace.csv" \
    $PYTHON -m vllm.entrypoints.openai.api_server --model "$MODEL" --port $PORT \
...
" | tee -a logs/ratesweep_fsx_THRESHOLDS.txt
}

$PYTHON scripts/hotpatch_fairshare_both.py || { log "PATCH FAILED"; touch logs/ratesweep_fsx_FAILED; exit 1; }
...
for WRATE in 0.5 1.0 2.0; do
  ...
  run_static "16384fsx${TAG}" --max-num-batched-tokens 16384
  run_static "512fsx${TAG}"   --max-num-batched-tokens 512
  run_fairshare_both "fsboth${TAG}"

  log "analyzing WRATE=$WRATE"
  SCHEDULE="$SCHED" ARMS="16384fsx${TAG} 512fsx${TAG} fsboth${TAG}" SLO_TBT_MS=500 \
    $PYTHON scripts/analyze_lengthgate.py > logs/ratesweep_fsx_${TAG}_ANALYSIS.txt 2>&1
  cat logs/ratesweep_fsx_${TAG}_ANALYSIS.txt
done

log "done -- see logs/ratesweep_fsx_{05,10,20}_ANALYSIS.txt and logs/ratesweep_fsx_THRESHOLDS.txt"
touch logs/ratesweep_fsx_ALLDONE
