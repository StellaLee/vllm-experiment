...
env GPU=0 TAG=convdiverse_conc10_widened_gpu0 CONC=10 WHALE_MIN=18000 BUDGETS='2048' TRIALS="$MISSING" PORT=8073 ./orchestrate/pesim/orchestrate_pesim_gate.sh >> $LOG 2>&1
echo "[$(STAMP)] Fill A done, exit=$?" >> $LOG

echo "[$(STAMP)] Fill B: CONC=6 widened, all 3 budgets, trials 2-10 (9 more each)" >> $LOG
env GPU=0 TAG=convdiverse_conc6_widened_gpu0 CONC=6 WHALE_MIN=18000 BUDGETS='16384 2048 512' TRIALS="$MISSING" PORT=8073 ./orchestrate/pesim/orchestrate_pesim_gate.sh >> $LOG 2>&1
...
env GPU=0 TAG=convdiverse_conc20_widened_gpu0 CONC=20 WHALE_MIN=18000 BUDGETS='16384 2048 512' TRIALS="$MISSING" PORT=8073 ./orchestrate/pesim/orchestrate_pesim_gate.sh >> $LOG 2>&1
