#!/bin/bash
# orchestrate_longprompt_depth.sh -- ablation: the DECODE-DEPTH + hysteresis controller under the exact
# long-prompt whale setup (same pad-seed 1001 -> whale positions paired with static/slocvar/ff arms).
# Question: does depth-gated chunking degenerate to mono (rails high) or chunk (rails low)? Depth is a
# COUNT not a COST, so it is blind to whale prefill cost -- this measures where it lands empirically.
# Outputs logs/<date>-longp-bdepth-t1.jsonl + chunktrace, then analyzes vs ALL prior arms.
set -uo pipefail
...
mkdir -p logs; rm -f logs/longpdepth_ALLDONE logs/longpdepth_FAILED
DATE=$(date +%Y-%m-%d)
...
FLOOR=${FLOOR:-512}; START=${START:-16384}; TARGET=${TARGET:-8}; HOLD=${HOLD:-3}
ARM=bdepth; PORT=8050
log "long-prompt DEPTH ablation: conc=$CONC whale_frac=$WHALE_FRAC whale=[$WHALE_MIN,$WHALE_MAX] floor=$FLOOR start=$START target=$TARGET hold=$HOLD"

log "  [$ARM] server on GPUs 0,1 port=$PORT budget=16384(max) mode=depth start=$START target=$TARGET"
env CUDA_VISIBLE_DEVICES=0,1 PREFIX_REORDER=0 \
    DYNAMIC_CHUNK=1 CHUNK_MODE=depth DYNAMIC_CHUNK_MIN=$FLOOR DYNAMIC_CHUNK_START=$START \
    DYNAMIC_CHUNK_TARGET=$TARGET DYNAMIC_CHUNK_HOLD=$HOLD \
...
for i in $(seq 1 120); do sleep 5
  ...
  [ "$i" = 120 ] && { log "  [$ARM] SERVER TIMEOUT"; kill "$SV" 2>/dev/null; touch logs/longpdepth_FAILED; exit 1; }
done
...
BUDGETS="16384 2048 512 ours ffv1 ffv2 depth" $PYTHON scripts/mlsys/analyze_longprompt.py > logs/longpdepth_ANALYSIS.txt 2>&1
echo "[$(STAMP)] DONE" >> logs/longpdepth_ANALYSIS.txt
touch logs/longpdepth_ALLDONE
log "done -> logs/longpdepth_ANALYSIS.txt"
