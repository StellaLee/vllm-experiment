#!/bin/bash
# rate_sweep_whale_aware_budget.sh -- real (non-oracle) whale-aware budget + decode-reserve,
# same non-stationary schedule as the oracle experiments, reusing their existing
# mono/static512 baselines and running only the new arm. Reports both phase-split and
# pooled (phase-agnostic) stats from the start.
set -uo pipefail
...
$PYTHON scripts/hotpatch_whale_aware_budget_v2.py || { log "PATCH FAILED"; touch logs/ratesweep_whaleawarev2_FAILED; exit 1; }
...
if [ "$UP" = 0 ]; then
  ...
  touch logs/ratesweep_whaleawarev2_FAILED; exit 1
fi
...
if rows:
    ...
    nwhales = [int(r[1]) for r in rows]
    n_hi = sum(1 for x in b if x == 512)
    print(f'[$ARM] n_rounds={n} hi_rounds={n_hi} ({100*n_hi/n:.1f}%) lo_rounds={n-n_hi} ({100*(n-n_hi)/n:.1f}%)')
    print(f'[$ARM] n_whales_active: min={min(nwhales)} p50={sorted(nwhales)[n//2]} max={max(nwhales)}')
else:
    print('[$ARM] NO TRACE ROWS')
" | tee -a logs/ratesweep_whaleawarev2_THRESHOLDS.txt

log "re-analyzing (phase-split): nsmono nsstatic512 ${ARM}"
OUT="logs/ratesweep_whaleawarev2_ANALYSIS.txt"
...
for label, path in [
    ...
    ('budget+reserve(oracle)', 'logs/${DATE}-lgate-bnsoraclebudgetreserve-t1.jsonl'),
    ('whale-aware-v1(real) ', 'logs/${DATE}-lgate-bnswhaleaware-t1.jsonl'),
    ('whale-aware-v2(real)     ', '${FB}-t1.jsonl'),
]:
    pooled(path, label)
" | tee -a "$OUT"

log "done -- see $OUT"
touch logs/ratesweep_whaleawarev2_ALLDONE
