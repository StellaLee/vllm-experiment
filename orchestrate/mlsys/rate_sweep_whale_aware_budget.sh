...
$PYTHON scripts/hotpatch_whale_aware_budget.py || { log "PATCH FAILED"; touch logs/ratesweep_whaleaware_FAILED; exit 1; }
...
if [ "$UP" = 0 ]; then
  ...
  touch logs/ratesweep_whaleaware_FAILED; exit 1
fi
...
" | tee -a logs/ratesweep_whaleaware_THRESHOLDS.txt

log "re-analyzing (phase-split): nsmono nsstatic512 ${ARM}"
OUT="logs/ratesweep_whaleaware_ANALYSIS.txt"
...
for label, path in [
    ...
    ('whale-aware(real)     ', '${FB}-t1.jsonl'),
]:
    pooled(path, label)
" | tee -a "$OUT"

log "done -- see $OUT"
touch logs/ratesweep_whaleaware_ALLDONE
