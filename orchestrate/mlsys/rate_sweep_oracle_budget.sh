#!/bin/bash
# rate_sweep_oracle_budget.sh -- oracle *aggregate budget* switch, same non-stationary
# schedule as rate_sweep_oracle_ns.sh, reusing its existing mono/static512 baselines
# (identical workload+seed) and running only the new oracle-budget arm.
set -uo pipefail
...
$PYTHON scripts/hotpatch_oracle_budget.py || { log "PATCH FAILED"; touch logs/ratesweep_oraclebudget_FAILED; exit 1; }
...
log "[$ARM] starting server (ORACLE_BUDGET)"
env CUDA_VISIBLE_DEVICES=0,1 PREFIX_REORDER=0 DYNAMIC_CHUNK=0 \
  ORACLE_BUDGET=1 ORACLE_LO_S=$LO_S ORACLE_HI_S=$HI_S \
...
if [ "$UP" = 0 ]; then
  ...
  touch logs/ratesweep_oraclebudget_FAILED; exit 1
fi
...
" | tee -a logs/ratesweep_oraclebudget_THRESHOLDS.txt

log "re-analyzing: nsmono nsstatic512 ${ARM}"
OUT="logs/ratesweep_oraclebudget_ANALYSIS.txt"
SCHEDULE="$SCHED" ARMS="nsmono nsstatic512 ${ARM}" SLO_TBT_MS=500 \
  $PYTHON scripts/analyze_lengthgate.py > "$OUT" 2>&1 || true
cat "$OUT"

log "done -- see $OUT"
touch logs/ratesweep_oraclebudget_ALLDONE
