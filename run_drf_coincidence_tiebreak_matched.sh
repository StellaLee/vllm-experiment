#!/bin/bash
# Hypothesis test: does the fleet-aggregate coincidence share (share_power_coincidence)
# beat drf_power_tiebreak's per-replica power share on the exact same throughput-matched
# whale-injection condition (rate=10.7, 750 convs, max-tokens 1024, whale-frac 0.15)?
...
OUTNAME=drf_coincidence_tiebreak
POLICY=drf_coincidence_tiebreak
for TRIAL in 1 2 3; do
  echo "=== DRFCOINCIDENCETIEBREAK BATCH: policy=$OUTNAME trial=$TRIAL ==="
  ...
  pkill -f "power_logger.py" 2>/dev/null
  sleep 3
  echo "=== DRFCOINCIDENCETIEBREAK BATCH: policy=$OUTNAME trial=$TRIAL COMPLETE ==="
done
echo "=== FULL DRFCOINCIDENCETIEBREAK BATCH COMPLETE ==="
