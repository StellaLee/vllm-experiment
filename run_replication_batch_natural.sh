...
for OUTNAME in $ARMS; do
  ...
  for TRIAL in 1 2 3; do
    echo "=== NATURAL BATCH: policy=$OUTNAME trial=$TRIAL ==="
    ...
    mkdir -p logs
    rm -f logs/natural_launch_${OUTNAME}_t${TRIAL}.log
    ...
    POLICY=$POLICY N_REPLICAS=6 GPU_OFFSET=2 MODEL=/data/pli/models/Qwen2.5-Coder-7B-Instruct \
      ...
      POWER_TRACE=logs/natural_power_trace_${OUTNAME}_t${TRIAL}.csv \
      ASSIGNMENT_LOG=logs/natural_assignment_${OUTNAME}_t${TRIAL}.csv \
      nohup bash orchestrate/eenergy/launch_router_experiment.sh > logs/natural_launch_${OUTNAME}_t${TRIAL}.log 2>&1 &
    LAUNCH_PID=$!
    ...
    timeout 900 python3 src/replay_sharegpt.py \
      ...
      --pad-chars 0 --whale-frac 0.15 --whale-min-chars 44000 --whale-max-chars 50000 \
      --request-timeout 180 \
      --output logs/natural_records_${OUTNAME}_t${TRIAL}.jsonl \
      2>&1 | tee logs/natural_harness_${OUTNAME}_t${TRIAL}.log
    ...
    pkill -f "power_logger.py" 2>/dev/null
    sleep 3
    echo "=== NATURAL BATCH: policy=$OUTNAME trial=$TRIAL COMPLETE ==="
  done
done
echo "=== FULL NATURAL REPLICATION BATCH COMPLETE ==="
