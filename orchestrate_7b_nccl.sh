...
calibrate(){ # label TP devices rates  -> echoes highest-bounded (capacity knee) rate
  ...
  env CUDA_VISIBLE_DEVICES=$dev PYTHON="$PYTHON" MODEL="$MODEL7B" \
      TP=$tp NUM=250 RATES="$rates" bash scripts/mlsys/calib_cs2b.sh > "$clog" 2>&1
...
sweep(){ # label TP devices rate
  ...
  env CUDA_VISIBLE_DEVICES=$dev PYTHON="$PYTHON" MODEL="$MODEL7B" \
      ...
      bash scripts/mlsys/run_cs2_repl.sh > "$LOGROOT/run-$label.log" 2>&1
...
# ---- 4. analyze ----
log "analyzing"
python scripts/mlsys/analyze_7b_nccl.py "$R_SINGLE" "$R_TP2" > logs/cs2_7bnccl_ANALYSIS.txt 2>&1
