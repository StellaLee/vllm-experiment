...
for OUTNAME in $ARMS; do
  ...
  for TRIAL in 1 2 3; do
    echo "=== CACHEHIT-HILOAD BATCH: policy=$OUTNAME trial=$TRIAL ==="
    ...
    mkdir -p logs
    rm -f logs/cachehit_hiload_launch_${OUTNAME}_t${TRIAL}.log
    ...
    POLICY=$POLICY N_REPLICAS=6 GPU_OFFSET=2 MODEL=/data/pli/models/Qwen2.5-Coder-7B-Instruct \
      ...
      POWER_TRACE=logs/cachehit_hiload_power_trace_${OUTNAME}_t${TRIAL}.csv \
      ASSIGNMENT_LOG=logs/cachehit_hiload_assignment_${OUTNAME}_t${TRIAL}.csv \
      nohup bash orchestrate/eenergy/launch_router_experiment.sh > logs/cachehit_hiload_launch_${OUTNAME}_t${TRIAL}.log 2>&1 &
    LAUNCH_PID=$!
    ...
    timeout 900 python3 src/replay_sharegpt.py \
      ...
      --concurrency 60 --num-convs 150 --max-tokens 128 \
      --pad-chars 0 --whale-frac 0.0 \
      --request-timeout 180 \
      --output logs/cachehit_hiload_records_${OUTNAME}_t${TRIAL}.jsonl \
      2>&1 | tee logs/cachehit_hiload_harness_${OUTNAME}_t${TRIAL}.log
    ...
    pkill -f "power_logger.py" 2>/dev/null
    sleep 3
    echo "=== CACHEHIT-HILOAD BATCH: policy=$OUTNAME trial=$TRIAL COMPLETE ==="
  done
done
echo "=== FULL CACHEHIT-HILOAD REPLICATION BATCH COMPLETE ==="
