...
print("\n=== Delta vs mono (thr=0), S population, full progression ===\n")
mtm, mt95, mt99, mpm, mp95, mp99, _ = results[("0 (mono/off)", "S")]
d = lambda a, b: (b - a) / a * 100 if a else float("nan")
for thr in ("512", "2048", "4096", "8192", "16384"):
    ctm, ct95, ct99, cpm, cp95, cp99, _ = results[(thr, "S")]
    print(f"thr={thr:>6}: dTTFT mean={d(mtm,ctm):+7.1f}% p95={d(mt95,ct95):+7.1f}% p99={d(mt99,ct99):+7.1f}% | "
          f"dTPOT mean={d(mpm,cpm):+7.1f}% p95={d(mp95,cp95):+7.1f}% p99={d(mp99,cp99):+7.1f}%")
