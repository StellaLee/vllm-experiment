#!/usr/bin/env python3
"""3-trial replication of the widened-distribution/Poisson-open-loop fleet-scale check
(cf_robustness_check.py ran only trial 1; this reruns per-trial with each trial's OWN
calibration + measured whale-window ramp rate, then reports mean+/-std across trials --
matching the paper's own replication convention for the main result).
...
analyze_power_shape_whale.py on each trial's own files):
  mono (b16384):  t1=50.3  t2=52.8  t3=53.5
  chunk=512:      t1=19.1  t2=31.8  t3=39.2
"""
import sys
import numpy as np
...
TRIALS = [
    dict(t=1, rec=f"{RAW}/2026-07-25-pesim_gate_wide_poisson-b16384-t1.jsonl",
         pw=f"{RAW}/2026-07-25-pesim_gate_wide_poisson-b16384-t1-power.csv",
         mono_ramp=50.3, chunk_ramp=19.1),
    dict(t=2, rec=f"{RAW}/2026-07-26-pesim_gate_wide_poisson-b16384-t2.jsonl",
         pw=f"{RAW}/2026-07-26-pesim_gate_wide_poisson-b16384-t2-power.csv",
         mono_ramp=52.8, chunk_ramp=31.8),
    dict(t=3, rec=f"{RAW}/2026-07-26-pesim_gate_wide_poisson-b16384-t3.jsonl",
         pw=f"{RAW}/2026-07-26-pesim_gate_wide_poisson-b16384-t3-power.csv",
         mono_ramp=53.5, chunk_ramp=39.2),
]
