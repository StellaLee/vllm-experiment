#!/usr/bin/env python3
"""Re-derivation of cf_reserve_tp2_14b.py using fit_kappa_self_consistent instead of a fixed
50s warmup. Investigates whether the warmup-insufficiency artifact found in
plot_traffic_pattern_profiles.py (cold-start OU transient not fully decayed within a fixed 50s
discard, confirmed to reach the same ORDER OF MAGNITUDE as the reported peak-ramp statistic
itself for slow-kappa conditions) also biases this script's real reserve-procurement numbers,
since TP=2/14B's fitted kappas (0.0489-0.0644, tau=15.5-20.4s) were among the slowest found --
5*tau up to ~102s, comparable to the old 50s-warmup + 100s-window = 150s total budget.

Everything else (trace tags, targets, N, T_window, n_mc) is unchanged from
cf_reserve_tp2_14b.py so the comparison isolates the effect of this one fix.
"""
import sys
...
MONO_TRIALS = [
    ("2026-07-30-pesim_gate_tp2_14b_wide-b16384-t1", 36.59),
    ("2026-07-30-pesim_gate_tp2_14b_wide-b16384-t2", 35.90),
    ("2026-07-30-pesim_gate_tp2_14b_wide-b16384-t3", 33.44),
]
CHUNK_TRIALS = [
    ("2026-07-30-pesim_gate_tp2_14b_wide-b512-t1", 27.03),
    ("2026-07-30-pesim_gate_tp2_14b_wide-b512-t2", 26.63),
    ("2026-07-30-pesim_gate_tp2_14b_wide-b512-t3", 26.14),
]
WHALE_THRESH = 15000
...
def main():
    ...
    print()
    print("=== TP=2/14B (FIXED: self-consistent kappa/warmup): mean +/- std across 3 trials ===")
