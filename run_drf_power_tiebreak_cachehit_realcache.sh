#!/bin/bash
# Same light/cachehit condition as run_drf_power_tiebreak_cachehit.sh (concurrency=24, plain
# multi-turn, no whale, max-tokens 128), but snapshots the REAL server-side prefix-cache hit
# rate (vLLM's own vllm:prefix_cache_hits_total / queries_total counters) right before
# teardown, to compare against the router's own cache_mirror.py estimate -- drf_power_tiebreak
# reported hit_rate% (mirror estimate) ~46.9+-1.0 for this condition in the existing table
# (findings/2026-08-31-eenergy-drf-lmetric-roundrobin-comparison.md). This run checks whether
# the real vLLM-side counters agree.
set -x
...
for TRIAL in 1 2 3; do
  echo "=== REALCACHECACHEHIT BATCH: policy=$OUTNAME trial=$TRIAL ==="
  ...
  pkill -f "power_logger.py" 2>/dev/null
  sleep 3
  echo "=== REALCACHECACHEHIT BATCH: policy=$OUTNAME trial=$TRIAL COMPLETE ==="
done
echo "=== FULL REALCACHECACHEHIT BATCH COMPLETE ==="
