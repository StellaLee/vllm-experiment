#!/usr/bin/env python3
"""Offline test of the fairshare-lpt patcher: apply to a scheduler FIXTURE, verify the block
lands right after the anchor, is idempotent, and refuses to stack on top of a different
already-installed controller patch. No vLLM/box needed."""
import os, sys, tempfile
sys.path.insert(0, os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "scripts", "mlsys"))
import hotpatch_fairshare_lpt as H
...
def test_patch_applies_fairshare_block():
    rc, out = _apply(FIXTURE)
    assert rc == 0
    assert 'if os.getenv("FAIRSHARE_LPT"):' in out
    # the whole formula: no calibrated constant anywhere, just budget / (1 + running)
    assert '_fs_thr = max(1, self.max_num_scheduled_tokens // (1 + _fs_running))' in out
    idx_anchor = out.index("token_budget = self.max_num_scheduled_tokens")
    idx_gate = out.index('if os.getenv("FAIRSHARE_LPT"):')
...
def test_patch_refuses_to_stack_on_other_controller():
    # Simulate a file already patched with a DIFFERENT controller (ADAPTIVE_LPT) -- fairshare
    # should refuse rather than silently stack a second block on the same anchor.
    d = tempfile.mkdtemp()
    p = os.path.join(d, "scheduler.py")
    ...
    out = open(p).read()
    assert rc == 1
    assert 'if os.getenv("FAIRSHARE_LPT"):' not in out
