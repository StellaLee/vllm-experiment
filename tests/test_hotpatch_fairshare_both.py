#!/usr/bin/env python3
"""Offline test of the fairshare-both patcher: verify it sets BOTH token_budget and
long_prefill_token_threshold from the same single formula, is idempotent, and refuses to stack
on another controller. No vLLM/box needed."""
import os, sys, tempfile
sys.path.insert(0, os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "scripts", "mlsys"))
import hotpatch_fairshare_both as H
...
def test_patch_sets_both_budget_and_threshold_from_same_value():
    rc, out = _apply(FIXTURE)
    assert rc == 0
    assert 'if os.getenv("FAIRSHARE_BOTH"):' in out
    assert '_fsx_share = max(1, self.max_num_scheduled_tokens // (1 + _fsx_running))' in out
    assert 'token_budget = _fsx_share' in out
    assert 'self.scheduler_config.long_prefill_token_threshold = _fsx_share' in out
    idx_anchor = out.index("token_budget = self.max_num_scheduled_tokens")
    idx_gate = out.index('if os.getenv("FAIRSHARE_BOTH"):')
...
def test_patch_refuses_to_stack_on_other_controller():
    ...
    out = open(p).read()
    assert rc == 1
    assert 'if os.getenv("FAIRSHARE_BOTH"):' not in out
