#!/bin/bash
# run_adaptive_lpt_arm2_replicate.sh -- Phase 1 replication of §5.7's adaptivelpt2 result
# (findings/2026-07-22-lengthgate-and-per-request-cap.md). Same phase-schedule workload,
# same gate/protect values, same seed (1001, matches this project's convention for
# phase-scheduled replications, e.g. §5.8) -- isolates serving/timing noise. TRIAL env var
# tags output so trial 1 (already on disk) is never touched.
set -uo pipefail
...
ARM=adaptivelpt2
FB="logs/${DATE}-lgate-b${ARM}-rep${TRIAL}"

rm -f "logs/lgate_adaptivelpt_rep${TRIAL}_ALLDONE" "logs/lgate_adaptivelpt_rep${TRIAL}_FAILED"

$PYTHON scripts/mlsys/hotpatch_adaptive_lpt.py || { log "PATCH FAILED"; touch "logs/lgate_adaptivelpt_rep${TRIAL}_FAILED"; exit 1; }

log "REPLICATION trial=$TRIAL arm=$ARM budget=16384 gate=$GATE protect=$PROTECT off=$OFF SCHED='$SCHED' DUR=${DUR}s"
...
for i in $(seq 1 120); do sleep 5
  ...
  [ "$i" = 120 ] && { log "SERVER TIMEOUT"; kill $SV 2>/dev/null; sleep 8; kill -9 $SV 2>/dev/null; kill_ours; touch "logs/lgate_adaptivelpt_rep${TRIAL}_FAILED"; exit 1; }
done
...
log "[$ARM trial=$TRIAL] recs=$(grep -c . ${FB}-t1.jsonl 2>/dev/null || echo 0) preempt=$(grep -c -i preempt ${FB}-server.log 2>/dev/null || echo 0)"
kill $SV 2>/dev/null; sleep 8; kill -9 $SV 2>/dev/null; kill_ours

touch "logs/lgate_adaptivelpt_rep${TRIAL}_ALLDONE"
log "trial $TRIAL done -> ${FB}-t1.jsonl"
